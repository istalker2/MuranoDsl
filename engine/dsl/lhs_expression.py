...
class LhsExpression(object):
    ...
    def _set_value(self, container, key, value, murano_class):
        if inspect.isgenerator(container):
            for t in container:
                self._set_value(t, key, value, murano_class)
        elif inspect.isgenerator(key):
            for t in key:
                self._set_value(container, t, value, murano_class)
        elif isinstance(container, types.DictionaryType):
            container[key] = value
        elif isinstance(container, MuranoObject):
            container.set_property(key, value, murano_class)
    ...
    def set(self, value, context, murano_class):
        if isinstance(self._expression, (yaql.expressions.Att,
            ...
            self._set_value(container, attr, value, murano_class)
